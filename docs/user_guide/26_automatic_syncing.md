# Setting Up Automatic Syncing (noGUI)

EPG data & providers update periodically. Automate syncing so data stays current. This process wil easily setup a sync schedule (noGUI) but requires Pro. Without Pro this must be done manually.

**Why automate**:
- Manual syncing is tedious
- EPG goes stale without updates
- Get latest provider updates with ease
