# How to Export or Download M3U File to Laptop

IPTVBoss generates files on cloud storage and locally.

### Question

How do I export or download the M3U file to my laptop?

### Answer

**To generate and access files:**

1. Set up cloud storage (Dropbox or Google Drive) in IPTVBoss settings
*Not required for local outputs, but recommended*
2. Output your layout: 
**Layout Manager → Output Current Layout M3U and EPG**
3. Files automatically upload to your cloud storage
4. Access via "View Cloud Links" or TinyURL
5. **Use these URLs in your player apps**
6. Files are also output locally to `IPTVBoss/output` folder

**To download the cloud file:**

- Open the cloud URL in your browser (it will download automatically)
- Or access files directly in your Dropbox/Google Drive folder

### Related Topics

- [Quick Start Guide](../Quick_Start.md)
- [TinyURL Links Not Generating](01_tinyurl_not_generating.md)
